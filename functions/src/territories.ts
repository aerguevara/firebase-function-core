...
interface TerritoryCell {
    ...
    activityId?: string;
    isHotSpot?: boolean;
    locationLabel?: string | null; // NEW: Store location label (FIXED type)
...
function createCell(x: number, y: number, now: Date, userId: string, activityId: string, expirationDays: number, locationLabel: string | null): TerritoryCell {
    ...
    return {
        ...
        locationLabel: locationLabel || null, // FIX: Use null instead of undefined to avoid Firestore crash
...
export const createProcessActivityComplete = (databaseId: string | undefined = undefined) =>
    onDocumentUpdated({
        ...
        try {
            ...
            // --- OPTIMIZATION: PRE-FETCH HOT SPOT STATUS IN PARALLEL ---
            const hotSpotMap = new Map<string, boolean>();
            const HOT_SPOT_CHUNK_SIZE = 50;

            console.log(`[Optimization] Pre-fetching Hot Spot status for ${cellIds.length} cells in chunks of ${HOT_SPOT_CHUNK_SIZE}...`);

            for (let i = 0; i < cellIds.length; i += HOT_SPOT_CHUNK_SIZE) {
                const chunk = cellIds.slice(i, i + HOT_SPOT_CHUNK_SIZE);
                await Promise.all(chunk.map(async (cellId) => {
                    try {
                        ...
                        if (recentOwners.size >= 3) {
                            hotSpotMap.set(cellId, true);
                        } else {
                            hotSpotMap.set(cellId, false);
                        }
                    } catch (e) {
                ...
                if (i > 0 && i % 200 === 0) {
                    console.log(`[Optimization] Processed ${i}/${cellIds.length} Hot Spot checks...`);
                }
            }
            // ------------------------------------------------------------
            for (const [cellId, newCell] of traversedCells.entries()) {
                ...
                if (existing) {
                    ...
                    if (existing.activityId === activityId) {
                        conquestCount++;
                        const ref = db.collection("remote_territories").doc(cellId);
                        const isHotSpot = hotSpotMap.get(cellId) || false;
                        const cellWithStatus = { ...newCell, lastInteraction: "conquest", isHotSpot };
            ...
            console.error(`CRITICAL ERROR processing activity ${activityId}:`, error);
            // Fallback: Try to update status to error so it doesn't get stuck in 'processing'
            try {
                const db = databaseId ? getFirestore(databaseId) : getFirestore();
