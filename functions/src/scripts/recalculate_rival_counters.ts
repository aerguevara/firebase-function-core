...
async function processDatabase(db: Firestore, dbName: string) {
    console.log(`\n🔵 Starting CUMULATIVE recalculation for database: ${dbName}`);

    // 1. Fetch all 'territory_stolen' notifications
    // We strictly need ALL of them to build the full history.
    console.log("   Fetching ALL theft notifications...");
    const snapshot = await db.collection("notifications")
        .where("type", "==", "territory_stolen")
        .get();

    console.log(`   Found ${snapshot.size} theft notifications.`);
    ...
    // 2. Aggregate Data
    // Map<ThiefId, Map<VictimId, {count, lastAt, ...}>>
    // Map<VictimId, Map<ThiefId, {count, lastAt, ...}>>
    ...
    for (const doc of snapshot.docs) {
        ...
        // Update Thief's "Recent Victims" data
        if (!victimMap.has(victimId)) {
            victimMap.set(victimId, {
                count: 1,
                lastAt: timestamp,
                details: { id: victimId, name: "...", avatar: null }
            });
        } else {
    ...
    console.log(`   Unique Thieves (with victims): ${thievesData.size}`);
    console.log(`   Unique Victims (with thieves): ${victimsData.size}`);

    // 3. Resolve Missing User Details (for Victims lists where we only have ID)
    ...
    const batchHandler = async (operations: Promise<any>[]) => {
        ...
        for (let i = 0; i < operations.length; i += chunkSize) {
            ...
            if ((i + chunkSize) % 500 === 0) process.stdout.write(".");
        }
        console.log("");
    };
